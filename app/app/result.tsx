...
export default function ResultScreen() {
    ...
    const { matchId, reset } = useGameStore()
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            const { data: questions } = await supabase
                .from('match_questions').select('id, question_text, correct_option, options')
                .eq('match_id', matchId).order('question_index', { ascending: true })
            const { data: answers } = await supabase
                .from('match_answers').select('player_id, question_id, user_answer').eq('match_id', matchId)
            ...
            setMyScore(me); setOpponentScore(opp); setBreakdown(bk); setMatchData(match); setLoading(false)
        }
        load()
    }, [matchId, session])
        ...
        <ScrollView style={s.container} contentContainerStyle={s.content}>
            ...
            {breakdown.map((item, i) => (
                <View key={i} style={s.breakdownRow}>
                    <Text style={s.breakdownIcon}>{item.correct === null ? '–' : item.correct ? '✅' : '❌'}</Text>
                    <Text style={s.breakdownText} numberOfLines={2}>{item.text}</Text>
                </View>
            ))}
            <View style={s.actions}>
                <TouchableOpacity style={s.primaryBtn} activeOpacity={0.85} onPress={() => { reset(); router.replace('/(tabs)') }}>
...
const makeStyles = (theme: any) => StyleSheet.create({
    ...
    heroTitle: { fontSize: 34, letterSpacing: 1 },
    ...
    scoreLabel: { fontSize: 11, color: theme.textSecondary, letterSpacing: 2, marginTop: 4 },
    ...
    payoutTitle: { fontSize: 12, color: theme.textSecondary, letterSpacing: 2, marginBottom: 8 },
    payoutAmount: { fontSize: 28, marginBottom: 8 },
    payoutTx: { color: theme.accent, fontSize: 13, fontFamily: 'monospace', textAlign: 'center' },
    ...
    sectionTitle: { fontSize: 11, color: theme.textSecondary, letterSpacing: 2, marginBottom: 12 },
    ...
    primaryBtn: { backgroundColor: theme.accent, borderRadius: 14, paddingVertical: 18, alignItems: 'center', shadowColor: theme.accent, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 8 },
    primaryBtnText: { color: '#fff', fontSize: 15, letterSpacing: 1.5 },
})
